...
// example_ids generate from example_ids_cfgs in data/Examples.xlsx
...
func (x *ExampleIds) MarshalObjectTo(buf []byte) (data []byte, err error) {
	data = buf
	if x.GoodsId != 0 {
...
func (x *ExampleIds) UnmarshalObject(data []byte) (err error) {
	index := 0
	for index < len(data) {
		...
		case 1:
			...
			if cnt < 1 {
				err = errors.New("parse ExampleIds.GoodsId ID:1 : invalid varint value")
				return
			}
			index += cnt
			x.GoodsId = uint32(v)
		case 2:
			v, cnt := protowire.ConsumeVarint(data[index:])
			if cnt < 1 {
				err = errors.New("parse ExampleIds.GoodsType ID:2 : invalid varint value")
				return
			}
			index += cnt
			x.GoodsType = int32(v)
		case 3:
			v, cnt := protowire.ConsumeVarint(data[index:])
			if cnt < 1 {
				err = errors.New("parse ExampleIds.DiamondPrice ID:3 : invalid varint value")
				return
			}
			index += cnt
		...
		case 4:
			v, cnt := protowire.ConsumeVarint(data[index:])
			if cnt < 1 {
				err = errors.New("parse ExampleIds.GoldPrice ID:4 : invalid varint value")
				return
			}
			index += cnt
			x.GoldPrice = int64(v)
...
func (x *ExampleIds) MarshalLogObject(enc zapcore.ObjectEncoder) error {
...
type ZapArrayExampleIds []*ExampleIds

func (x ZapArrayExampleIds) MarshalLogArray(ae zapcore.ArrayEncoder) error {
	for _, v := range x {
		ae.AppendObject(v)
	}
	return nil
...
func LogArrayExampleIds(name string, v []*ExampleIds) zap.Field {
	return zap.Array(name, ZapArrayExampleIds(v))
}

// example_ids generate from example_ids_cfgs in data/Examples.xlsx
type ExampleIdsContainer struct {
	Data []*ExampleIds `json:"data,omitempty"` // table
}

func (x *ExampleIdsContainer) Reset() {
	*x = ExampleIdsContainer{}
}

// MarshalObject marshal data to []byte
func (x *ExampleIdsContainer) MarshalObject() (data []byte, err error) {
	data = make([]byte, 0, x.MarshalSize())
	return x.MarshalObjectTo(data)
}

// MarshalSize calc marshal data need space
func (x *ExampleIdsContainer) MarshalSize() (size int) {
	if x.Data != nil {
		// 1 = protowire.SizeTag(1)
		size += 1 * len(x.Data)
...
// MarshalObjectTo marshal data to []byte
func (x *ExampleIdsContainer) MarshalObjectTo(buf []byte) (data []byte, err error) {
	data = buf
	if x.Data != nil {
...
// UnmarshalObject unmarshal data from []byte
func (x *ExampleIdsContainer) UnmarshalObject(data []byte) (err error) {
	index := 0
	for index < len(data) {
		num, typ, cnt := protowire.ConsumeTag(data[index:])
		if num == 0 {
		...
		case 1:
			if typ != protowire.BytesType {
				err = errors.New("parse ExampleIdsContainer.Data ID:1 : invalid repeated tag value")
				return
			}
			buf, cnt := protowire.ConsumeBytes(data[index:])
			if buf == nil {
				err = errors.New("parse ExampleIdsContainer.Data ID:1 : invalid len value")
				return
			}
			index += cnt
			if x.Data == nil {
				x.Data = make([]*ExampleIds, 0, 2)
			}
			item := &ExampleIds{}
...
func (x *ExampleIdsContainer) MarshalLogObject(enc zapcore.ObjectEncoder) error {
...
type ZapArrayExampleIdsContainer []*ExampleIdsContainer

func (x ZapArrayExampleIdsContainer) MarshalLogArray(ae zapcore.ArrayEncoder) error {
	for _, v := range x {
		ae.AppendObject(v)
	}
	return nil
...
func LogArrayExampleIdsContainer(name string, v []*ExampleIdsContainer) zap.Field {
	return zap.Array(name, ZapArrayExampleIdsContainer(v))
}
